    ...
    try:
        ...
        if global_info_path.exists():
            ...
            if spec and spec.loader:
                ...
                logging.debug(
                    "Successfully loaded global_info from %s", global_info_path
                )
                return {
...
def create_eidosian_json(base_path: Path, repo_name: str) -> bool:
    ...
    eidosian_data = {
        ...
        "metadata": {
            ...
            "version_history": [
                {"version": "1.0.0", "date": now_iso, "description": "Initial creation"}
            ],
        },
        "self_analysis": {
...
def create_configuration_files(
    ...
    eidosian_config = render_template(
        EIDOSIAN_CONFIG_TEMPLATE,
        {
            "repo_name": repo_name,
            "current_date": datetime.now().strftime("%Y-%m-%d"),
    ...
    license_content = (
        global_info["license"]
        if isinstance(global_info["license"], str)
        else render_template(
            LICENSE_TEMPLATE, {"current_year": datetime.now().strftime("%Y")}
        )
    )
    ...
    # Create SECURITY.md
    security = render_template(
        SECURITY_TEMPLATE,
        {"repo_name": repo_name, "author_email": global_info["author"]["email"]},
    )
    write_file(base_path / "SECURITY.md", security)
    created_files.append("SECURITY.md")

    # Create CHANGELOG.md
    changelog = render_template(
        CHANGELOG_TEMPLATE,
        {"current_date": datetime.now().strftime("%Y-%m-%d"), "repo_name": repo_name},
    )
    ...
    if "python" in languages:
        try:
            ...
            from gis_forge.global_info import generate_pyproject_toml

            pyproject_content = generate_pyproject_toml(
                project_name=repo_name, description=f"Eidosian {repo_name} repository"
            )
            write_file(base_path / "pyproject.toml", pyproject_content)
            created_files.append("pyproject.toml")
        except ImportError:
            logging.debug(
                "Could not import generate_pyproject_toml, skipping pyproject.toml generation"
            )
